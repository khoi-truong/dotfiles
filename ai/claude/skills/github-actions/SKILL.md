...
- **cost-and-speed**: Linux only (macOS bills 10×). Don't run `push` + `pull_request` on the same branch. Filter aggressively so a docs PR costs ~0. Cache the build cache for wall-clock; cache-key fields ordered `<item>-<os>-<hash>-<sha>` (item name first). Cheap gates first so failures surface fast.
