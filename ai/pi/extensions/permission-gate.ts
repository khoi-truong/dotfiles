...
// Differs from upstream: more patterns, blocks headless, and also gates powershell.
/**
 ...
 * adversary. Known gaps: interpreter one-liners (python -c, node -e),
 * $(...) and backticks, and anything else that hides the command text.
 */

import { type ExtensionAPI, isToolCallEventType } from "@earendil-works/pi-coding-agent";

export const dangerousPatterns = [
  // rm with any flag group containing r/R, or --recursive (but not git rm -r:
  // it deletes tracked files, but git can restore them)
  /(?<!\bgit([ \t]+-\S+([ \t]+[^-\s]\S*)?)*[ \t]+)\brm\s+(-\S+\s+)*(-\w*[rR]|--recursive)/,
  /\bsudo\b/i,
  /\b(chmod|chown)\b.*777/i,
  /\bbrew\s+(bundle(?!\s+(check|list)\b)|install|reinstall|uninstall|remove|upgrade)\b/i,
  /\bfind\b.*\s-(delete|exec(dir)?\s+(\S*\/)?rm)\b/,
