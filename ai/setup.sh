...
# --- secrets ---------------------------------------------------------------
# Two gitignored files, both sourced by ai/aliases.zsh:
#
#   env.secrets.zsh  generated below from 1Password. Never hand-edit.
#   env.local.zsh    yours, for anything not in that item.
#
...
SECRETS="${CURRENT_DIR}/env.secrets.zsh"
...
if ! command -v op >/dev/null 2>&1; then
  warn "1password-cli not found — skipping ai/env.secrets.zsh"
elif ! command -v jq >/dev/null 2>&1; then
  warn "jq not found — skipping ai/env.secrets.zsh"
  ...
  (
    umask 077
    {
      printf '%s\n' \
        ...
        "# item ${OP_ITEM}). Hand edits are lost on the next run — put those" \
        "# in ai/env.local.zsh instead. Gitignored; mode 600." \
        ""
      printf '%s\n' "$op_exports"
    } >"${SECRETS}.tmp"
  )
