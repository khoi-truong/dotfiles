...
5. `zsh/aliases.zsh`, `zsh/aliases.macos.zsh`, `zsh/functions.zsh` — after
   plugins, so these win.
...
7. `ai/aliases.zsh` (which sources `ai/claude/providers.zsh`) — after mise,
   because the launcher cache `providers.zsh` keeps is regenerated by the
   `python3` mise activation puts on `$PATH`.
