...
(`-U` replaces an existing key, so the same command rotates it); remove it
with `security delete-generic-password -a "$USER" -s deepseek-api-key`.
pi reads the key once per run, so restart it after a change. A
